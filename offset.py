...
st.write("**Base:**")
st.write(f"P1 → X = {mostrar_3_decimales(x1)}, Y = {mostrar_3_decimales(y1)}")
st.write(f"P2 → X = {mostrar_3_decimales(x2)}, Y = {mostrar_3_decimales(y2)}")

st.write("**Offset:**")
st.write(f"P1′ → X = {mostrar_3_decimales(P1_offset[0])}, Y = {mostrar_3_decimales(P1_offset[1])}")
st.write(f"P2′ → X = {mostrar_3_decimales(P2_offset[0])}, Y = {mostrar_3_decimales(P2_offset[1])}")
...
st.write(f"**Ángulo interno:** {grados_a_dms(angulo)}")

if abs(angulo - 90) < 0.01:
    st.success("Ángulo paralelo: 90° 00' 00\"")
else:
    desviacion = abs(angulo - 90)
    st.warning(f"Desviación: {grados_a_dms(desviacion)}")
